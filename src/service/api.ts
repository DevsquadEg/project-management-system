...
export const TASK_URLS = {
  ...
  GET_TASK: (id:any) => `/Task/${id}`, // Get task by id
  UPDATE_TASK: (id:any) => `/Task/${id}`, // Update task by id
  DELETE_TASK: (id:any) => `/Task/${id}`, // Delete task by id
  COUNT_TASKS: "/Task/count", // Count tasks for manager and employee
  CHANGE_STATUS: (id:any) => `/Task/${id}/change-status`, // Change task status
  GET_TASKS_BY_PROJECT: (id:any) => `/Task/project/${id}`, // Get tasks by project id
};
